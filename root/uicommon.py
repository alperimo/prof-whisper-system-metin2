...
class QuestionDialogtm(ui.ScriptWindow):
...
class QuestionDialog2tm(QuestionDialogtm):

	def __init__(self):
		QuestionDialogtm.__init__(self)
		self.__CreateDialog()

	def __del__(self):
		QuestionDialogtm.__del__(self)
...
class Soru(QuestionDialog2tm):
	...
	def __del__(self):
		QuestionDialog2tm.__del__(self)

	def __CreateDialog(self):
		pyScrLoader = ui.PythonScriptLoader()
		pyScrLoader.LoadScriptFile(self, "uiscript/questiondialogriot.py")
	...
	def SetTextBaslik(self, text):
		self.textLine1.SetText(text)
		
	def Buton1ad(self, text):
		self.acceptButton.SetText(text)
		
	def Buton2ad(self, text):
		self.cancelButton.SetText(text)
		
	def SetKapatEvent(self, event):
		self.kapatButton.SetEvent(event)

	def OnUpdate(self):
		leftTime = max(0, self.endTime - app.GetTime())
		
class SoruRT(QuestionDialog2tm):
	...
	def __CreateDialog(self):
		pyScrLoader = ui.PythonScriptLoader()
		pyScrLoader.LoadScriptFile(self, "uiscript/questiondialogriotrt.py")
		...
		self.cancelButton = self.GetChild("cancel")
		self.kapatButton = self.GetChild("kapat")
		self.sonrakiButton = self.GetChild("sonraki")
		self.oncekiButton = self.GetChild("onceki")
	...
	def SetSonrakiEvent(self, event):
		self.sonrakiButton.SetEvent(event)
		
	def SetOncekiEvent(self, event):
		self.oncekiButton.SetEvent(event)

	def OnUpdate(self):
		leftTime = max(0, self.endTime - app.GetTime())
		
class SikayetRT(QuestionDialog2tm):
	...
	def __CreateDialog(self):
		...
		self.oncekiButton.Hide()
		self.sonrakiButton.Hide()
		
		self.textLine2.SetText(localeInfo.SIKAYETETVEYAENGELLEXD)
		
		self.board.SetSize(280+100, 105+10)
		
		self.acceptButton.SetUpVisual("locale/ui/config/spam1.tga")
		self.acceptButton.SetOverVisual("locale/ui/config/spam2.tga")
		self.acceptButton.SetDownVisual("locale/ui/config/spam2.tga")
		self.acceptButton.SetToolTipText(localeInfo.SIKAYETET)
		self.acceptButton.SetText("")
		
		self.cancelButton.SetUpVisual("locale/ui/config/spam1_e.tga")
		self.cancelButton.SetOverVisual("locale/ui/config/spam2_e.tga")
		self.cancelButton.SetDownVisual("locale/ui/config/spam2_e.tga")
		self.cancelButton.SetToolTipText("Sohbetten Engelle")
...
class InputDialogWithDescription(InputDialog):
	...
	def OnUpdate(self):
		riotInfo.BiyologAdet = self.inputValue.GetText()
...
class QuestionDialogWithTimeLimitRiot(QuestionDialog2):
...
class MoneyInputDialog(ui.ScriptWindow):
	...
	def SetAcceptText(self, text):
		self.acceptButton.SetText(str(text))
		
	def SetCancelText(self, text):
		self.cancelButton.SetText(str(text))
...
class MoneyInputDialogRT3(ui.ScriptWindow):
	...
	def __OnValueUpdate(self):
		...
		if text and text.isdigit():
			try:
				...
				money = 1999999999
		riotInfo.RTGirilen = text
		self.moneyText.SetText("Senin RT : " + str(constInfo.RTPuan))
		
class MoneyInputDialogRT2(ui.ScriptWindow):
	...
	def __OnValueUpdate(self):
		...
		if text and text.isdigit():
			try:
				...
				money = 1999999999
		riotInfo.RTGirilen = text
		self.moneyText.SetText("Senin NP : " + str(constInfo.Np))
		
class MoneyInputDialogRT(ui.ScriptWindow):
	...
	def __OnValueUpdate(self):
		...
		riotInfo.RTGirilen = text
		if text == "":
			self.moneyText.SetText(localeInfo.NumberToMoneyString(money) + " RT / " + str(riotInfo.RTPuan))
		else:
			self.moneyText.SetText(localeInfo.NumberToMoneyString(money) + " RT / " + str(riotInfo.RTPuan))
...
class MoneyInputDialog2(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		...
		pyScrLoader = ui.PythonScriptLoader()
		pyScrLoader.LoadScriptFile(self, "uiscript/moneyinputdialog2.py")

		getObject = self.GetChild
		self.board = self.GetChild("board")
		self.acceptButton = getObject("AcceptButton")
		self.cancelButton = getObject("CancelButton")
		self.itemiste = getObject("ItemButton")
		self.inputValue = getObject("InputValue")
		self.inputValue.SetNumberMode()
		self.inputValue.OnIMEUpdate = ui.__mem_func__(self.__OnValueUpdate)
		self.moneyText = getObject("MoneyValue")
	...
	def itemiste(self, event):
		self.itemiste.SetEvent(event)
		self.inputValue.OnIMEReturn = event
